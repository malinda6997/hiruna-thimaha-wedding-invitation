...
const VIEW_W = 1200;
const VIEW_H = 340; // Increased height to fit larger wave ribbon
const CX = VIEW_W / 2;
const CY = VIEW_H / 2;
const EDGE_PAD = 10;
...
const buildPath = (shape: TextLoopShape, curviness: number, ribbonWidth: number): string => {
  ...
  switch (shape) {
    ...
    case 'arch': {
      const rise = Math.min(80 + c * 1.1, room * 2);
...
const TextLoop = ({
  ...
  ribbon = true,
  ribbonColor = '#7e22ce',
  ribbonWidth = 110,   // Increased ribbon height/thickness
  pauseOnHover = true,
  className = '',
  style = {}
}: TextLoopProps) => {
  ...
  return (
    <div ref={rootRef} className={`relative w-full overflow-hidden select-none ${className}`.trim()} style={style}>
      <svg
